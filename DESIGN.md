...
Hero correction after user review: the left panel is the clean product photograph and the right panel is a distinct category-matched campaign/lifestyle image, matching the reference page's actual visual grammar. The campaign panel uses a gentle full-frame drift rather than the previous close crop of the same product photo. Existing original campaign assets are reused.
