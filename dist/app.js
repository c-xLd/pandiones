...
function showProduct(id){location.href='product.html?id='+encodeURIComponent(id)}
...
$('#add-form').addEventListener('submit',e=>{e.preventDefault();const size=new FormData(e.target).get('size');if(!size)return;const found=bag.find(i=>i.id===selected&&i.size===size);if(found)found.qty++;else bag.push({id:selected,size,qty:1});renderBag();const productDialog=$('#product-dialog');if(productDialog?.open)productDialog.close();$('#toast').textContent=products[selected].name+' çantana eklendi.';$('#toast').classList.add('show');clearTimeout(toastTimer);toastTimer=setTimeout(()=>$('#toast').classList.remove('show'),3500)});
