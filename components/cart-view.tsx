...
export default function CartView() {
  ...
  if (!cart?.items.length) return <div className="cart-empty-v2"><span>ÇANTAN / 00</span><ShoppingBag className="cart-empty-icon" aria-hidden="true" /><h2>Henüz bir seçim<br /><i>yapmadın.</i></h2><p>Koleksiyondan sana eşlik edecek parçaları seç; bedenini belirle ve çantana ekle.</p><Button asChild size="lg"><a href="/koleksiyon">Koleksiyonu keşfet <ArrowRight aria-hidden="true" /></a></Button><div className="cart-empty-links"><a href="/ic-giyim">İç Giyim</a><a href="/koleksiyon/kategori/crop-bustiyer">Crop Büstiyer</a><a href="/beden-rehberi">Beden Rehberi</a></div></div>;

  return (
    <div className="cart-layout-v2">
      <section className="cart-items-v2" aria-label="Çantadaki ürünler">
        <header><div><span>SEÇİMLERİN</span><strong>{cart.count.toString().padStart(2, '0')} PARÇA</strong></div><a href="/koleksiyon">Alışverişe devam et <ArrowRight aria-hidden="true" /></a></header>
        {cart.items.map((item, index) => {
          const pending = pendingId === item.id;
          return <article className={`cart-item-v2 ${pending ? 'is-pending' : ''}`} key={item.id}>
            <a className="cart-item-media-v2" href={`/urun/${item.slug}`}><img src={item.image} alt={`${item.name}, ${item.color}`} /><span>0{index + 1}</span></a>
            <div className="cart-item-main-v2">
              <div className="cart-item-title-v2"><p>{item.color} / {item.size} BEDEN</p><h2><a href={`/urun/${item.slug}`}>{item.name}</a></h2><span>Birim fiyat · {money(item.priceKurus)}</span></div>
      ...
      <aside className="cart-summary-v2">
        ...
        <div className="cart-summary-links-v2"><a href="/teslimat-iade">Teslimat & İade <ArrowRight aria-hidden="true" /></a><a href="/beden-rehberi">Beden Rehberi <ArrowRight aria-hidden="true" /></a></div>
      </aside>
