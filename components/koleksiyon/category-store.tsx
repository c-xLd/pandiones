  ...
  return (
    <main className={`shop-page category-store category-store--${slug}`}>
      ...
      <section className="category-signature" aria-labelledby="category-title">
        ...
        {feature && <Link className="category-signature-image" href={`/${feature.slug}`} aria-label={`${feature.name} ürününü incele`} prefetch={true}><img src={feature.image} alt={feature.name} style={{ objectPosition: feature.imagePosition }} loading="eager" decoding="async" /><span>01 / ÖNE ÇIKAN</span></Link>}
      </section>
      ...
      {products.length > 0 ? (
        <section className="category-editorial-products" id="kategori-urunleri" aria-label={`${title} ürünleri`}>
          ...
          {products.map((product, index) => <article className={`category-editorial-card category-editorial-card--${index + 1}`} key={product.id}><Link className="category-editorial-media" href={`/${product.slug}`} prefetch={true}><img src={product.image} alt={`${product.name}, ${product.color}`} style={{ objectPosition: product.imagePosition }} loading={index < 3 ? 'eager' : 'lazy'} decoding="async" /><span>0{index + 1}</span><b>İncele ↗</b></Link><div><div><p>{product.categoryName} / {product.color}</p><h3><Link href={`/${product.slug}`} prefetch={true}>{product.name}</Link></h3></div><strong>{formatPrice(product.priceKurus)}</strong></div></article>)}
        </section>
      ) : (
